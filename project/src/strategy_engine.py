...
class MultiAssetStrategyEngine:
    ...
    def generate_signal(
        ...
        overbought = indicators["rsi"] > 72 or indicators["stoch_k"] > 88
        oversold = indicators["rsi"] < 28 or indicators["stoch_k"] < 12
        if overbought:
            bull -= 1
            confidence -= 10
        ...
        overextended = indicators["distance_sma20_pct"] > max(
            profile.high_atr_pct, atr_pct * profile.overextension_atr_multiple
        )
        if overextended:
            confidence -= 12
        ...
        if bull >= 5 and gap >= 2 and confidence >= profile.min_confidence and not overbought and not overextended:
        ...
        if bear >= 5 and gap >= 2 and confidence >= profile.min_confidence and not oversold:
